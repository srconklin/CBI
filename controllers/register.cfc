component accessors=true extends="controllers.base.common" {
	...
	************************************************************/
	public void function completeprofile(struct rc = {}) {
		...
		****************************************************************/
			...
			// see their profile
			 ...
			 if (!register.markUserFullyValidated()) 
				status = handleServerError(rc, register.getErrorContext());
			 else {
		...
		******************************************/
		if(listFindNoCase('defaultCP,successfullyVerified,emailAlreadyVerified,successfullyVerifiedButNeedPassword,emailAlreadyVerifiedButNeedPassword', status))
		...
		param name="rc.bcast" default="0";
			...
			*/
			...
			if(register.hasErrors()) {
				...
				variables.userService.updateUserState(email=register.getEmail(), state={'pwdVerified' : true})
	...
	private string function sendPasswordlink(struct rc = {}) {
		var status ='';
				...
				if (forgotpassword.hasErrors()) 
					status = handleServerError(rc, forgotpassword.getErrorContext());
	...
	************************************************************/
	private string function verifyEmail(struct rc = {}) {
		...
		// yeah! token is valid so we are authenticated
			...
			variables.userService.updateUserState(email=register.getEmail(), state={'verifyVerified' : 1})
	...
	************************************************************/
	private string function verifyEmailForPwd(struct rc = {}) {
		...
		if (!fp.verifyToken()) {
			//error
			status  = handleServerError(rc, fp.getErrorContext());
