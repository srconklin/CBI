			...
			<div class="carousel-sizer"  >
				...
				<div class="carousel-prev">
					<button 
					  ...
					  x-on:click="$store.carousel.activeSlide = $store.carousel.activeSlide === 0 ? $store.carousel.slides.length-1 : $store.carousel.activeSlide - 1; $store.carousel.direction = 'leftoright'">
				...
				<template x-for="(slide, index) in $store.carousel.slides" :key="index">
					<div style="overflow:hidden;">
						...
						x-show="$store.carousel.activeSlide === index"
						x-transition:enter="carousel-animate" 
						:x-transition:enter-start="$store.carousel.direction == 'leftoright' ? 'transform -translate-x-full' : 'transform translate-x-full'"
				...
				<div class="carousel-next">
				  <button 
					class="carousel-button"
					x-on:click="$store.carousel.activeSlide = $store.carousel.activeSlide === $store.carousel.slides.length-1 ? 0 : $store.carousel.activeSlide + 1; $store.carousel.direction = 'righttoleft'">
					<svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor">
